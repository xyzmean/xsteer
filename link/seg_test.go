...
func TestРазборОтвергаетЧужое(t *testing.T) {
	...
	badSum[20+16] ^= 0xFF
	_, ok := ParseSeg(badSum)
	if !ok {
		t.Error("сегмент с битой суммой отвергнут — тогда зондировщика хаб не увидит")
	}
	// Сама функция суммы по-прежнему проверяется — просто ParseSeg её больше не зовёт (там
	// сказано почему). Считаем её здесь напрямую: у собранного нами сегмента она даёт нуль, у
	// испорченного — нет.
	if Csum(*(*[4]byte)(badSum[12:16]), *(*[4]byte)(badSum[16:20]), badSum[20:]) == 0 {
		t.Error("испорченный сегмент даёт нулевую сумму — проверять её нечем")
	}
	if Csum(*(*[4]byte)(good[12:16]), *(*[4]byte)(good[16:20]), good[20:]) != 0 {
		t.Error("верная сумма не даёт нуля")
	}
