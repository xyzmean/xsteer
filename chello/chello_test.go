...
func TestСоставИРазмеры(t *testing.T) {
	...
	// Hello обязан быть БРАУЗЕРНОГО размера, то есть НЕ влезать в один сегмент.
	//
	// Это перевёрнутое требование: раньше здесь стояло «обязан влезать». Сравнение с настоящим
	// трафиком (tests/xhttp-compare.sh) показало, что современный Chrome отдаёт Hello на ~1760
	// байт двумя сегментами — из-за постквантового ключа X25519MLKEM768, — а наши 537 байт одним
	// сегментом опознаются и по размеру, и по числу сегментов. У эталона Xray с отпечатком Chrome
	// в том же замере вышло 1759 байт, у нас теперь столько же.
	if len(rec) != 1759 {
		t.Errorf("Hello вышел %d байт, а браузерный — 1759", len(rec))
	}
	// Без постквантового обмена (режим совместимости с хабом на C) Hello возвращается к прежнему
	// размеру, и это тоже надо знать числом.
	small, err := BuildOpt("www.microsoft.com", true, false, carrier(0x77), seeded{rand.New(rand.NewSource(42))})
	if err != nil {
		t.Fatal(err)
	}
	if len(small) != 537 {
		t.Errorf("Hello без постквантового обмена вышел %d байт, а был 537", len(small))
	}
	if _, err := Parse(small); err != nil {
		t.Errorf("Hello без постквантового обмена не разбирается: %v", err)
	}
}
...
func TestЗамороженныйСлепок(t *testing.T) {
	...
	// Слепок сменился НАРОЧНО, когда в key_share добавился постквантовый обмен: без него Hello
	// занимал 537 байт против 1759 у настоящего Chrome. Число сверено с эталоном Xray в том же
	// замере, а не выведено из соображений.
	const want = "4f1e81f7deda8a79"
	if got != want {
	...
	// Длина Hello — 1742 байта плюс длина SNI. Сверено с настоящим Chrome через эталон Xray: у
	// него в том же замере вышло 1759 при SNI из 17 символов, у нас столько же.
	if want := 1742 + len("www.microsoft.com"); len(rec) != want {
		t.Errorf("длина Hello %d, а браузерная — %d", len(rec), want)
	}
}
