...
func TestПоддельныйSYNНеСбрасываетСессию(t *testing.T) {
	...
	// Зато проба запущена: SYN в живую сессию — единственный внешний признак того, что пир мог
	// вернуться с той же четвёркой, и проверить это надо, а не отбросить и забыть.
	if st.s.roamCheck.Load() == 0 {
		t.Error("поддельный SYN не запустил пробу пути: вернувшийся пир с той же четвёркой ждал бы " +
			"смерти прежней сессии по простою, то есть минуты")
	}
	...
	// И проба, получив ответ, снимается сама: живую сессию она убить не может.
	//
	// Порядок здесь важен и повторяет живой: сначала обслуживание отправляет пробу, и только потом
	// приходит ответ. Проверять «трафик был до пробы» бессмысленно — до пробы он был у любой
	// сессии, включая мёртвую.
	st.w.maintain()
	if st.s.roamCheck.Load() == 0 {
		t.Fatal("проба снялась, не отправив ни одной попытки")
	}
	third := ip4Frame(ip4(10, 0, 0, 2), ip4(10, 0, 0, 3), 400)
	st.feedRecord(third, 1400)
	st.w.maintain()
	if st.s.roamCheck.Load() != 0 {
		t.Error("проба не снялась, хотя сессия ответила")
	}
	if st.w.sess[st.k] == nil {
		t.Error("живая сессия убрана пробой — а она отвечает")
	}
}

// TestПробаПутиСнимаетМолчащуюСессию: та же проба на сессии, по которой подтверждённого трафика
// больше нет.
//
// ЗАЧЕМ. Слот в наборе соединений пира выбирается хешем потока, поэтому слот, ведущий в точку,
// которой больше нет, — это не задержка, а потеря доли потоков целиком. Прежде такая сессия жила до
// IdleMS, то есть три минуты, и всё это время часть трафика уезжала в никуда. Проба обязана снять
// её за полсекунды — и снять в первую очередь ПРИВЯЗКУ, а не только запись в таблице.
func TestПробаПутиСнимаетМолчащуюСессию(t *testing.T) {
	st := newStand(t)
	s := st.s
	if st.h.peerSess[s.peer][s.connID].Load() != s {
		t.Fatal("сессия не привязана к пиру — стенд собран не так")
	}
	// Пир умолк: подтверждённого трафика с этой секунды нет вовсе.
	s.lastAuth.Store(nowMS() - 60000)
	s.roamCheck.Store(nowMS())

	// Обслуживание идёт раз в сто миллисекунд; проба — три попытки через roamProbeMS.
	for i := 0; i < roamProbes+2 && st.w.sess[st.k] != nil; i++ {
		st.w.maintain()
		time.Sleep(roamProbeMS * time.Millisecond)
	}
	if st.w.sess[st.k] != nil {
		t.Fatal("молчащая сессия не снята пробой")
	}
	if got := st.h.peerSess[s.peer][s.connID].Load(); got != nil {
		t.Errorf("привязка пир→сессия осталась (%p) — часть трафика продолжит уезжать в мёртвую "+
			"точку", got)
	}
}

// TestПростойСчитаетсяПоПодтверждённомуТрафику: посторонний сегмент в четвёрку сессии не считается
// признаком жизни.
//
// Прежде время последнего приёма обновлял КАЖДЫЙ принятый сегмент, включая нерасшифрованный, — а на
// публичный порт хаба сыплется сканерный шум. Значит сессия, в которую её пир давно не пишет, могла
// жить сколько угодно: достаточно, чтобы кто-то посторонний бил в её четвёрку. Слот в наборе пира
// она при этом занимала.
func TestПростойСчитаетсяПоПодтверждённомуТрафику(t *testing.T) {
	st := newStand(t)
	s := st.s
	s.lastAuth.Store(nowMS() - IdleMS - 1000)

	// Посторонний сегмент с данными, которые не расшифруются: время приёма соединения он обновит.
	junk, ok := link.ParseSeg(st.mkSeg(standISN+1, link.PSH|link.ACK, []byte("не наша запись")))
	if !ok {
		t.Fatal("сегмент не разобрался")
	}
	st.w.onSeg(&junk)
	if got := s.conn.Idle(); got > 1000 {
		t.Fatalf("стенд собран не так: соединение считает простой %d мс", got)
	}
	if got := s.idleMS(); got < IdleMS {
		t.Fatalf("простой сессии считается по постороннему сегменту: %d мс", got)
	}
	st.w.maintain()
	if st.w.sess[st.k] != nil {
		t.Error("сессия без подтверждённого трафика не убрана: её держит живой посторонний шум")
	}
}
