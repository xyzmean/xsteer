...
func (h *Hub) streamConn(ctx context.Context, nc net.Conn) {
	...
	for {
		...
		// ВСПЛЕСК КОНЧИЛСЯ ЗДЕСЬ: следующая запись придёт из ReadRecord, который блокирует, то
		// есть ждать её можно сколько угодно долго. Без этого вызова последний пакет всплеска
		// лежал бы в накопителе разгрузки до следующей записи — то есть до следующего запроса
		// того же пира, а при одном открытом соединении и тихом канале просто навсегда.
		//
		// Раньше сброса здесь не было вовсе, и работало это лишь на потоковой загрузке: накопитель
		// отдаёт содержимое сам, когда полон. То есть чем меньше трафика, тем дольше висел пакет —
		// ровно наоборот тому, что нужно человеку.
		if w.dev != nil {
