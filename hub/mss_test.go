...
// TestПотолокMTUУстройства — I-117: MTU устройства брался из настроек без потолка.
//
// Разбор конфигурации принимает MTU до conf.LinkMax (1500) — это предел КАНАЛА, — а на устройство
// туннеля уезжает то же число, у которого потолок mtuCeil (1439). tunLoop при этом читает окнами
// ровно wire.MTUDefault, и пакет больше окна ядро усекает МОЛЧА: прочитано 1439 байт, в заголовке
// IP заявлено 1500, ни ошибки, ни признака (проверено пробой на ядре 6.8). Получатель выбросил бы
// такой пакет как испорченный.
//
// Правило и число обязаны совпадать с клиентом (client.clampMTU): расхождение означало бы, что
// одна половина одного туннеля принимает настройку, которую другая зажимает. Стенд закрепляет
// именно это — таблицу значений, а не факт наличия функции.
func TestПотолокMTUУстройства(t *testing.T) {
	cases := []struct{ in, want int }{
		{0, mtuCeil},       // не задано — потолок, дальше согласование опустит
		{-1, mtuCeil},      // мусор — туда же
		{1500, mtuCeil},    // MTU КАНАЛА вместо туннеля: обычная описка, и раньше она проходила
		{1440, mtuCeil},    // на один байт выше потолка
		{mtuCeil, mtuCeil}, // ровно потолок
		{1400, 1400},       // меньше потолка — как задано
		{576, 576},         // нижняя граница, какую принимает разбор
	}
	for _, c := range cases {
		if got := clampMTU(c.in); got != c.want {
			t.Errorf("clampMTU(%d) = %d, хочу %d", c.in, got, c.want)
		}
	}
}
