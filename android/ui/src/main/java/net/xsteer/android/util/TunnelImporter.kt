...
import net.xsteer.android.fragment.ConfigNamingDialogFragment
...
object TunnelImporter {
    ...
    /**
     * Принять то, что человек принёс: файл настройки текстом ИЛИ ссылку xs://.
     *
     * Ссылку разбирает движок, а не приложение, и это не лень: разбирать её второй раз на Kotlin
     * значило бы иметь два мнения о формате и разойтись с ними при первой же правке. Движок
     * принимает оба вида и отдаёт обратно всегда текст, с которым дальше работает обычный разбор.
     */
    fun importTunnel(parentFragmentManager: FragmentManager, configText: String, messageCallback: (CharSequence) -> Unit) {
        try {
            val text = if (configText.trimStart().startsWith("xs://", ignoreCase = true)) {
                val probe = xsteer.Xsteer.newTunnel()
                probe.configure(configText.trim())
                probe.confText()
            } else configText
            ...
            ConfigNamingDialogFragment.newInstance(text).show(parentFragmentManager, null)
        } catch (e: Throwable) {
    ...
    private const val TAG = "xsteer/TunnelImporter"
}