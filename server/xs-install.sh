...
# СЕРВЕРЫ ИМЁН — не украшение, а условие работы полного туннеля.
#
# Пир с AllowedIPs = 0.0.0.0/0 уводит в туннель ВЕСЬ трафик, включая запросы к резолверу своей
# сети — а он обычно живёт на адресе домашней сети (192.168.x.1) и через туннель недостижим. Имена
# перестают разрешаться, и выглядит это как «интернета нет», хотя туннель поднят и трафик идёт.
# Поэтому такому пиру в конфигурацию пишется DNS, и адрес должен быть публичным.
#
# Пиру, который заворачивает только звезду, DNS НЕ пишется: его интернет идёт напрямую, и менять
# ему резолвер всей системы — не дело туннеля.
function askDNS() {
	echo ""
	echo "Серверы имён для пиров, которые заворачивают ВЕСЬ трафик."
	echo "Без них имена на телефоне перестанут разрешаться: резолвер своей сети останется прежним,"
	echo "а идти ему через туннель некуда."
	echo "   1) Cloudflare — 1.1.1.1, 1.0.0.1"
	echo "   2) Quad9 — 9.9.9.9, 149.112.112.112"
	echo "   3) Google — 8.8.8.8, 8.8.4.4"
	echo "   4) свои"
	echo "   5) не писать — пир разберётся сам"
	local d=""
	until [[ ${d} =~ ^[1-5]$ ]]; do ask d "Выбор [1-5]: " 1; done
	case "$d" in
	1) HUB_DNS="1.1.1.1, 1.0.0.1" ;;
	2) HUB_DNS="9.9.9.9, 149.112.112.112" ;;
	3) HUB_DNS="8.8.8.8, 8.8.4.4" ;;
	5) HUB_DNS="" ;;
	4) HUB_DNS=""
	   until [ -n "$HUB_DNS" ]; do
		ask HUB_DNS "Адреса через запятую: " "1.1.1.1, 1.0.0.1"
		# Проверяется КАЖДЫЙ адрес: имя вместо адреса разбор конфигурации отвергнет, и узнает об
		# этом человек уже на пире, отказом при подъёме.
		local bad=0 one
		for one in ${HUB_DNS//,/ }; do
			[[ ${one} =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]] || { echo -e "${RED}«$one» не адрес IPv4${NC}"; bad=1; }
		done
		[ "$bad" = 0 ] || HUB_DNS=""
	   done ;;
	esac
...
# У хаба, поставленного до появления этого вопроса, в params нет строки HUB_DNS. Спрашиваем один
# раз при первой же выдаче пира и дописываем — иначе такой хаб молча выдавал бы полные туннели без
# резолвера, то есть ровно ту беду, ради которой вопрос и появился.
function ensureDNS() {
	[ -n "${HUB_DNS+x}" ] && return 0
	askDNS
	printf 'HUB_DNS=%s\n' "${HUB_DNS}" >> "$PARAMS"
}
...
[Interface]
PrivateKey = ${ppriv}
Address    = ${PEER_IP}/${HUB_PLEN}${PEER_DNS:+
DNS        = ${PEER_DNS}}
