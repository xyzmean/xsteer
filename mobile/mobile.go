...
//
// СОЕДИНЕНИЕ К ХАБУ ЗДЕСЬ РОВНО ОДНО, и это свойство именно этого входа. Дескриптора у туннеля
// нет вовсе: пакеты и туда, и оттуда ходят вызовами через один объект системы
// (NEPacketTunnelFlow на iOS), то есть узкое место — сам этот объект, а не число соединений.
// Развести пакеты по нескольким соединениям значило бы только переставить их порядок, ничего не
// выиграв: очередь всё равно одна и разбирает её один вызывающий.
...
//
// СОЕДИНЕНИЙ К ХАБУ ЗДЕСЬ СТОЛЬКО ЖЕ, СКОЛЬКО НА ОБЫЧНОМ LINUX: по одному на ядро, но не больше
// предела. Долго стояла единица с доводом «пакеты и туда, и оттуда идут через один объект
// системы»; для iOS это правда, для дескриптора — нет, и цена ошибки видна в замере.
//
// Путь данных здесь тот же, что на десктопе, с одной разницей: разгрузки сегментации нет.
// IFF_VNET_HDR ставится при создании устройства, а создаёт его VpnService, и попросить его об
// этом нечем. Значит на приёме каждый пакет уезжает в ядро отдельным write(2), и в профиле
// клиента эти записи — половина всего процессорного времени (замер: 84% времени в системных
// вызовах, из них половина — запись в устройство). При одном соединении вся эта работа лежит на
// ОДНОЙ горутине, то есть на одном ядре телефона; при нескольких — раскладывается по ядрам, и
// раскладывает её хаб по хешу потока, так что пакеты одного соединения TCP всегда едут одним
// путём и переставлены не бывают.
//
// Перестановка на ОТДАЧЕ при этом возможна и замером НАЙДЕНА: очередь устройства одна, читают её
// все воркеры, и хеша потока на этой стороне нет. Цена зависит от задержки пути, и зависимость
// эта противоположная тому, чего ждёшь. На пути с задержкой 40 мс — то есть на любом настоящем
// пути телефона — четыре соединения дали втрое больше отдачи (1697..1954 против 557..567 Мбит/с)
// и в пять раз МЕНЬШЕ повторных передач внутреннего TCP (1780..8931 против 43955..55413): одно
// соединение там упиралось в своё окно, и переставленные пакеты стоили дешевле, чем простой.
// А на стенде, где задержки нет вовсе (оба конца на одной машине), четыре соединения отдачу
// вдвое УХУДШИЛИ (1491..1523 против 3000..3180) и повторов добавили. Такого пути у телефона не
// бывает, но знать про него надо: числа и условия — в шапке tests/phone.sh.
//
// Цена решения названа честно: соединений четыре, значит четыре сокета, четыре рукопожатия при
// подъёме и четыре пробы живости раз в две секунды вместо одной. Для радио телефона это
// ощутимо меньше, чем разница в скорости, ради которой всё и делается.
...
// start — общая часть обоих входов. Два способа заполучить устройство отличаются самим
// устройством и числом соединений к хабу; всё остальное — настройки клиента, режим потока,
// слежение за сетью — обязано у них совпадать, и совпадает оно потому, что написано здесь один
// раз.
//
// conns — сколько соединений открыть к хабу. Ноль означает «по одному на ядро, но не больше
// предела». Число приходит СВЕРХУ, от того входа, которым туннель подняли, и это не мелочь: оно
// зависит не от системы, а от того, что за устройство нам дали, — см. объяснение у каждого входа.
func (t *Tunnel) start(makeDev func(mtu int) (tun.Device, error), conns int) error {
	t.mu.Lock()
	if t.cancel != nil {
		t.mu.Unlock()
	...
	opt := client.Options{
		...
		Devs: []tun.Device{dev},
		// Соединений столько, сколько назвал вход. Почему не всегда одно — в комментариях у Start
		// и StartFD; здесь важно только то, что это решение принимается не по названию системы.
		Conns: conns,
