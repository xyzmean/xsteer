...
type Client struct {
	...
	// noV6 — сколько кадров IPv6 из устройства отброшено, noV6At — когда об этом сказано в
	// последний раз. noIP и noIPAt — то же для кадров, которые не IP вовсе: короче заголовка IPv4
	// или с другой версией в первом байте.
	//
	// Считаем отдельно от общего счётчика потерь (stats.dropped), и в него они НЕ попадают: это не
	// потеря пути, а кадр, которому в нашем туннеле некуда идти, и путать их в разборе беды дорого.
	// Раздельно между собой — потому что IPv6 от телефона это штатный фон, а мусор из устройства —
	// признак поломки того, кто устройство кормит.
	noV6   atomic.Uint64
	noV6At atomic.Int64
	noIP   atomic.Uint64
	noIPAt atomic.Int64
...
// tunnelable — годится ли кадр из устройства в туннель. Негодный сосчитан и назван в журнале;
// вызывающий его просто пропускает.
//
// КАДР, КОТОРОМУ НЕКУДА ИДТИ, ОТБРАСЫВАЕТСЯ ЗДЕСЬ, а не у хаба.
//
// Адресация туннеля у нас только IPv4: и конфигурация, и таблица маршрутов знают четыре байта.
// IPv6 хаб отбрасывает («маршрутизации IPv6 в хабе нет»), но ДО этого такой кадр успевает съесть
// место в записи, уйти по каналу и — хуже всего — сойти за активную отправку. А сторож пути судит
// именно по ней: «шлём, а в ответ тишина дольше DeadMS» означает мёртвый путь, и соединение
// поднимается заново.
//
// На телефоне это не теория. Android постоянно проверяет связь по IPv6 и шлёт эти пакеты в
// туннель; хаб их выбрасывает и отвечать ему нечем — то есть клиент сам себе устраивал
// переподключение раз в минуту, и пользовательский трафик не успевал пойти вовсе.
//
// ОДНА ФУНКЦИЯ НА ОБА ТРАНСПОРТА. Прежде проверка стояла только в outbound (поддельный TCP), а
// телефон работает потоком — то есть там, ради кого она писалась, её не было.
func (c *Client) tunnelable(f []byte) bool {
	if len(f) >= wire.IPHdr && f[0]>>4 == 4 {
		return true
	}
	if len(f) >= 40 && f[0]>>4 == 6 {
		c.warnDrop(&c.noV6, &c.noV6At,
			"кадры IPv6 в туннель не идут: адресация туннеля только IPv4 (отброшено %d)")
	} else {
		c.warnDrop(&c.noIP, &c.noIPAt,
			"из устройства пришли кадры, которые не IPv4 и не IPv6, — отбрасываю (отброшено %d)")
	}
	return false
}

// warnDrop считает отброшенный кадр и говорит об этом — но не чаще раза в минуту.
//
// Раз в минуту, а не на каждый кадр: стек телефона шлёт их пачками (link-local, multicast), и
// строка на каждый превратила бы журнал в шум, из-за которого не видно остального.
func (c *Client) warnDrop(cnt *atomic.Uint64, at *atomic.Int64, msg string) {
	n := cnt.Add(1)
	now := nowMS()
	last := at.Load()
	if now-last < 60_000 && last != 0 {
		return
	}
	if !at.CompareAndSwap(last, now) {
		return
	}
	c.logf(msg, n)
}
...
func Run(ctx context.Context, opt Options) error {
	...
	// ownDevs — открыли ли устройство мы сами. От этого зависит, значит ли число устройств
	// «столько очередей дало ядро» (наш случай) или «столько объектов дал встраивающий» (чужой).
	ownDevs := len(opt.Devs) == 0
	devs := opt.Devs
	if len(devs) == 0 {
		open := tun.OpenQueues
		if opt.NoOffload {
	...
	c.devs = devs
	name := devs[0].Name()
	// ЧИСЛО СОЕДИНЕНИЙ ЗАЖИМАЕТСЯ ЧИСЛОМ УСТРОЙСТВ ТОЛЬКО ТОГДА, КОГДА УСТРОЙСТВА ОТКРЫЛИ МЫ.
	//
	// В нашем случае len(devs) — это столько очередей, сколько дало ядро на IFF_MULTI_QUEUE, и
	// воркеров больше очередей заводить незачем: каждая очередь уже приходит с готовым разбором
	// по потокам, а два воркера на одной очереди этот разбор сломали бы.
	//
	// А вот устройство, отданное встраивающим, — совсем другое дело. Оно ровно одно и другим быть
	// не может: на Android система выдаёт один дескриптор от VpnService, на iOS — один объект, и
	// попросить второй нечем. Зажимать по нему соединения означало «на телефоне соединение всегда
	// одно», и это ровно то, что происходило: mobile.go просил четыре, а работало одно, потому что
	// устройство одно. Весь приём при этом — расшифровка и по одному write(2) на пакет, разгрузки
	// сегментации у чужого дескриптора нет — лежал на ОДНОЙ горутине, то есть на одном ядре
	// телефона. Сколько соединений нужно, знает встраивающий, и спорить с ним здесь нечем.
	//
	// Плата названа прямо: несколько воркеров на одной очереди читают её вперемешку, то есть
	// отдача может уехать переставленной (обратное направление раскладывает хаб по хешу потока,
	// и там перестановки нет). Замер на стенде телефонного пути (tests/phone.sh) её не показал.
	if conns > len(devs) && (ownDevs || accumulates(devs[0])) {
...
// accumulates — копит ли устройство пакеты в супер-кадр (разгрузка сегментации включена).
//
// Такое устройство НЕЛЬЗЯ делить между воркерами: накопитель у него один, замка на нём нет и не
// должно быть — на своём пути данных к нему обращается ровно одна горутина, и замок стоил бы
// ровно того выигрыша, ради которого разгрузка и существует. Отданные встраивающим устройства
// разгрузки не имеют по построению (IFF_VNET_HDR ставится при создании, а создаёт устройство не
// он), но проверка здесь дешевле, чем разбирательство с испорченным супер-кадром потом.
func accumulates(d tun.Device) bool {
	o, ok := d.(interface{ Offload() (bool, string) })
	if !ok {
		return false
	}
...
func (c *Client) outbound(ctx context.Context, id int, s *sess, dev tun.Device) {
	...
	for ctx.Err() == nil {
		...
		for len(frames) < max {
			...
			f := slab[used : used+n]
			// Кадр, которому в туннеле некуда идти, отбрасывается здесь, а не у хаба (см.
			// tunnelable). Место в slab не занимается: used не сдвигается.
			if !c.tunnelable(f) {
				continue
			}
...
func (c *Client) probeDone(s *sess, id int, dev tun.Device, devName string) {
	...
	if s.pLo <= wire.MTUFloor {
		...
		s.mtuConfirmed = 0
		// ПОДТВЕРЖДЁННЫМ СТАНОВИТСЯ НИЗ, а не остаётся прежнее (I-278). Кадр итога ниже уходит
		// только по нулевому соединению; остальные называют хабу mtuPub и называют его заново,
		// лишь когда он меняется (s.mtuTold != pub в timeLoop). Оставленный прежним — скажем,
		// 1439 после повторного пробоя под живой сессией, — он держал их сессии на хабе на 1439:
		// хаб резал по нему MSS обратного трафика и слал полноразмерные кадры в сузившийся путь.
		// Низ здесь честное «подтверждено»: он считается заведомо проходящим, проба такого размера
		// не отправляется вовсе. Ноль сюда не годится: при нуле остальные соединения не называют
		// ничего, и хаб оставил бы их сессиям согласованное в рукопожатии.
		//
		// На переподключении это значит: s.mtuConfirmed = низ, поэтому probeStart начинает полный
		// поиск, а не проверку прежнего; ронять устройство на низ «как на первом рукопожатии» уже
		// не нужно — оно там и стоит.
		c.mtuPub.Store(int64(wire.MTUFloor))
