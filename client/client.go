...
type Client struct {
	...
	// noV6 — сколько кадров IPv6 отброшено, noV6At — когда об этом сказано в последний раз.
	// Считаем отдельно от общего счётчика потерь: это не потеря пути, а кадр, которому в нашем
	// туннеле некуда идти, и путать их в разборе беды дорого.
	noV6   atomic.Uint64
	noV6At atomic.Int64
...
// warnNoV6 говорит про отброшенные кадры IPv6 — но не чаще раза в минуту.
//
...
func (c *Client) warnNoV6() {
	...
	if !c.noV6At.CompareAndSwap(last, now) {
		return
	}
	c.logf("кадры IPv6 в туннель не идут: адресация туннеля только IPv4 (отброшено %d)", n)
}
...
func (c *Client) outbound(ctx context.Context, id int, s *sess, dev tun.Device) {
	...
	for ctx.Err() == nil {
		...
		for len(frames) < max {
			...
			//
			// Адресация туннеля у нас только IPv4: и конфигурация, и таблица маршрутов знают
			// четыре байта. IPv6 хаб отбрасывает («маршрутизации IPv6 в хабе нет»), но ДО этого
			// такой кадр успевает съесть место в записи, уйти по каналу и — хуже всего —
			// сойти за активную отправку. А сторож пути судит именно по ней: «шлём, а в ответ
			// тишина дольше DeadMS» означает мёртвый путь, и соединение поднимается заново.
			//
			// На телефоне это не теория. Android постоянно проверяет связь по IPv6 и шлёт эти
			// пакеты в туннель; хаб их выбрасывает и отвечать ему нечем — то есть клиент сам
			// себе устраивал переподключение раз в минуту, и пользовательский трафик не успевал
			// пойти вовсе.
			if n < 20 || f[0]>>4 != 4 {
