...
// РАЗМЕРЫ БУФЕРОВ СОКЕТА МЫ НЕ ЗАДАЁМ, И ЭТО РЕШЕНИЕ, А НЕ УПУЩЕНИЕ.
//
// Соблазн понятен: на канале с большим произведением полосы на задержку окно TCP упирается в
// буфер, и хочется попросить у ядра побольше. Так и было сделано — SetReadBuffer/SetWriteBuffer на
// два мегабайта, — и вышло РОВНО НАОБОРОТ, потому что явный setsockopt отключает автоподстройку
// буфера и заодно режется по net.core.{r,w}mem_max, а он по умолчанию 212992. Ядро удваивает
// запрошенное, и на живом сервере получилось 425984 байт вместо автоподстроечных четырёх мегабайт:
// при задержке 72 мс это потолок 47 Мбит/с против 466. Замерено на хабе, где рядом работала
// прежняя сборка: у неё tb4194304, у новой tb425984.
//
// Автоподстройка Linux растёт до tcp_rmem/tcp_wmem (6 и 4 МБ) сама и делает это по фактическому
// произведению полосы на задержку — то есть лучше любой нашей постоянной. На Windows и Android
// автоподстройка тоже есть. Поэтому единственная правильная величина здесь — та, которую мы не
// задаём; оператору, которому нужен потолок выше, поднимать net.core.{r,w}mem_max, а не нам
// пришпиливать буфер в коде.
