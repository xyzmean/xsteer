...
const (
	...
	// туннеля), но полный туннель С ИСКЛЮЧЕНИЯМИ — обычное дело: «весь трафик, кроме локальных
	// и корпоративных сетей» раскладывается калькулятором AllowedIPs в два-три десятка
	// префиксов (0.0.0.0/0 минус 10/8, 172/8, 100/8 — уже девятнадцать). Шестнадцати на это не
	// хватало, и конфиг самого частого сценария отвергался. Шестьдесят четыре покрывают любой
	// разумный список исключений; таблица маршрутов — просто срез, платить за запас нечем.
	AllowedMax = 64
...
func Parse(text []byte, role Role) (*Conf, *Secrets, error) {
	...
	for i, rawLine := range raw {
		...
		// Комментарий в конце строки: «#» после пробела или табуляции и всё за ним. Так пишет
		// сам пример в steer/docs/xsteer.md («MTU = 1439  # необязательно…») и так читает
		// wg-quick. Разбор на C (xsconf.c) снимает его точно так же — две реализации одного
		// формата обязаны принимать один и тот же файл. «#» вплотную к значению («1400#x»)
		// комментарием не считается: это хвост значения, и строгий разбор числа его отвергнет.
