...
func ParseLink(s string, role Role) (*Conf, *Secrets, string, error) {
	...
	var allowed string
	// Имя параметра приводится к нижнему регистру ДО проверки на повтор, а не в switch ниже.
	//
	// Иначе повтор другим регистром не ловится вовсе: одинаковые имена url.ParseQuery складывает в
	// один срез (его и проверяет len(vs) > 1), а «pk» и «PK» остаются ДВУМЯ ключами map — то есть
	// проверка их не видит, а switch после ToLower присваивает одно и то же поле дважды. И это хуже,
	// чем «последний победил»: порядок обхода map в Go случайный, поэтому один и тот же текст ссылки
	// давал бы то один ключ хаба, то другой, от запуска к запуску (замерено: 200 разборов одной
	// строки дали оба ключа, 23 против 177). Половина на C приводит имя к нижнему регистру до своей
	// проверки и такую ссылку отвергает — правильное поведение здесь её, а расхождение означало бы,
	// что «настроено» зависит от того, какой половиной настраивали.
	...
	for k, vs := range q {
		...
		if prev, dup := seen[lk]; dup {
			return nil, nil, "", fmt.Errorf("параметр %s задан дважды (%s и %s) — какое значение "+
				"верное, ссылка не говорит", lk, prev, k)
		}
		seen[lk] = k
