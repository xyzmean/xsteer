...
func TestСсылкаОтвергает(t *testing.T) {
	...
	cases := []struct{ name, link, want string }{
		...
		// Повтор ДРУГИМ РЕГИСТРОМ — тот же повтор: имя параметра разбор читает без учёта
		// регистра, поэтому «pk=…&PK=…» — это дважды pk, а не два разных параметра. Проверяется
		// отдельными случаями, потому что ловится это не тем же кодом: одинаковые имена склеивает
		// url.ParseQuery в один срез, разные — оставляет двумя ключами map.
		{"повтор pk другим регистром", base + "&PK=" + KeyEncodeURL(mustKey(t, privB64)), "дважды"},
		{"повтор ip другим регистром", base + "&IP=10.99.0.7/24", "дважды"},
		{"повтор sni другим регистром", base + "&sni=a&SNI=b", "дважды"},
...
// TestИмяПараметраБезРегистра — имя параметра читается без учёта регистра, и один раз в любом
// написании — это НЕ повтор. Стенд стоит рядом с проверкой повтора нарочно: приведение имени к
// нижнему регистру до проверки легко превратить в отказ на «PK=…», которое обязано работать. То же
// правило у половины на C (xslink.c приводит имя к нижнему регистру перед сравнением).
func TestИмяПараметраБезРегистра(t *testing.T) {
	link := "xs://" + KeyEncodeURL(mustKey(t, privB64)) + "@203.0.113.7:443?PK=" +
		KeyEncodeURL(mustKey(t, pubB64)) + "&IP=10.77.0.2/24&Sni=example.org"
	c, s, _, err := ParseLink(link, RoleSpoke)
	if err != nil {
		t.Fatalf("ссылка с именами параметров в другом регистре не принята: %v", err)
	}
	defer s.Wipe()
	if c.Peers[0].Pub != mustKey(t, pubB64) || c.AddrPlen != 24 || c.SNI != "example.org" {
		t.Errorf("разобрано не то: %+v", c)
	}
}
